...
// einaregilsson's Javascript port of Peter Norvig's excellent Sudoku solver.
// See http://norvig.com/sudoku.html for his article and explanations.
// See https://github.com/einaregilsson/sudoku.js/blob/master/norvig/sudoku-norvig.js for git repo
...
function solve(grid) {
	var values = search(parseGrid(grid));
	display(values);
	return values;
}
...
function main() {
	...
	var values = solve(randomPuzzle(15));
	var results = vals(values);
	//Results is digit array of the solved puzzle
	//randomPuzzle will return the initial puzzle. The parameter is the number of starting digits requested.
